...
<script>
	...
	function pullData() { 
		table = $('#tabel').DataTable({
			...
			"order"   : [],
			"ajax"    : {
				"url" : "<?= site_url('Satuanbarang/getAll') ?>", 
				"type": "POST"
			},
			"columnDefs"   : [{
				"targets"  : [1, 3, 4, 5],
                ...
                "targets": [-1],
                "data": null,
                "defaultContent": '<div class="me-3"><button type="button" class="btn btn-edit" data-bs-toogle="tooltip" data-bs-placement="top" title="Edit"><i data-feather="edit" class="text-info" alt="img"></i></button> || <button type="button" class="btn btn-delete" data-bs-toogle="tooltip" data-bs-placement="top" title="Delete"><i data-feather="trash-2" class="text-danger" alt="img"></i></button></div>',
            },{
                "width": "10%",
                "targets": [0]
            }],
    ...
    function tambah(){
        ...
        $('#addsatuanmodal').on('shown.bs.modal', function(event){
            $(this).find('form')[0].reset();
        });

        $('#addsatuanmodal').modal({
            backdrop    : 'static',
            keyboard    :  false
        });
        ...
        $('#addsatuanmodal').on('hidden.bs.modal', function(){
            $(this).find('form')[0].reset();
        ...
        $.ajax({
            ...
            success : function(response){
                if (response.success == true){
                    $('#addsatuanmodal').on('shown.bs.modal', function(event){
                        $('#dataId').val(response.result.idSatuan);
