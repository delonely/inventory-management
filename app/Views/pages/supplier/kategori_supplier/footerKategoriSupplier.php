...
<script>
	...
	function pullData() { 
		table = $('#tabel').DataTable({
			...
			"order"   : [],
			"ajax"    : {
				"url" : "<?= site_url('Kategorisupplier/getAll') ?>", 
				"type": "POST"
			},
			"columnDefs"   : [{
				"targets"  : [1, 2, 5],
	...
	function edit(id){
		$.ajax({
			type: "get",
			url: "<?=site_url('Kategorisupplier')?>/" + id,
			data: {
				id: id,
			},
			dataType: "json",
			success: function(response){
				if (response.success == true){
					...
					$('#addkategorimodal').on('shown.bs.modal', function(event){
						...
						$('#dataId').val(response.result.idKategoriSupplier);
	...
	function remove(id){
		$.ajax({
			type: "delete",
			url: "<?=base_url('Kategorisupplier')?>/" + id,
			dataType: "json",
			success: function(response){
				if (response.success == true){
					Swal.fire({
